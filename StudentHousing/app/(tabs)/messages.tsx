...
import { StorageContext } from "../auth_provider";

export default function MessagesScreen() {
  const storage = useContext(StorageContext);

  useEffect(() => {
    console.log(storage?.state.swipedHistory);
    console.log(storage?.state.conversations);
  }, [storage?.state.swipedHistory]);
    ...
    <SafeAreaView className="flex-1">
      <View className="flex-1 h-screen px-4">
        <FlatList
          ...
          ListHeaderComponent={
            <View>
              ...
              <FlatList
                ...
                data={storage?.state.swipedHistory}
                horizontal
          ...
          data={storage?.state.conversations}
