...
export const getUserData = async (userId, { minimal, exclude, sourceId, recommender, mode = "flatmate", filters } = {})=>{
  ...
  if (exclude) {
    ...
    const { data: _userData, error: _userError } = await supabase.from("profile_mapping").select(`
          ...
          id,
          ${getUserDataQuery}
        `).filter("id", "not.in", `(${userId.join(",")})`).filter("type", "eq", mode);
    userData = _userData;
    ...
    console.log("getting mapping", mode, userId, _userData);
    userData = _userData;
    userError = _userError;
  }
...
export const getConnectedUserIds = async (userId, mode, type = "flatmate")=>{
  let query = supabase.from("connections").select(`cohert2, cohert1`);
  if (type === "accommodation") {
    query = query.eq("cohert1", userId);
  } else {
    query = query.or(`cohert1.eq.${userId},cohert2.eq.${userId}`);
  }
...
export const getLikedUserIds = async (userId)=>{
  const { data: likeData, error: likeError } = await supabase.from("profile_interactions").select("cohert2").eq("cohert1", userId);
  // .eq("type", "like")
  if (likeError) {
    return {
      status: "error",
...
export const getFilteredUserIds = async (filters, excludeUserIds = [], mode)=>{
  ...
  parsedFilters.forEach(([key, value])=>{
    if (key === "budget") {
      ...
      const [min, max] = value;
      query = query.eq("key", key).gte(`value->data->value`, min).lte(`value->data->value`, max);
  ...
  return {
    response: filteredUserIds
  };
};
export const getDiscoveryProfiles = async (sourceId, filters, search = "flatmate")=>{
  const { response: connectedUsersIds } = await getConnectedUserIds(sourceId, search);
