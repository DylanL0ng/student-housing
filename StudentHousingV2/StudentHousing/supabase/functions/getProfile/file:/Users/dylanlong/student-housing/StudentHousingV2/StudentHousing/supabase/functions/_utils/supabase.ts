...
export const getPromptLabel = async (prompt_id)=>{
  ...
  const { data: promptData, error: promptError } = await supabase.from("prompt_registry").select("prompt").eq("id", prompt_id).single();
  if (promptError) {
...
export const getAllPrompts = async ()=>{
  const { data: promptData, error: promptError } = await supabase.from("prompt_registry").select("id, prompt");
  if (promptError) {
...
export const getUserData = async (userId, { minimal, exclude, sourceId, recommender, mode = "flatmate" } = {})=>{
  ...
  if (exclude) {
    ...
    const { data: _userData, error: _userError } = await supabase.from("profiles").select(`
          full_name,
          type,
          id,
          ${getUserDataQuery}
    ...
    const { data: _userData, error: _userError } = await supabase.from("profiles").select(`
              ...
              id,
              ${getUserDataQuery}
              `).filter("type", "eq", mode).in("id", userId);
    userData = _userData;
    userError = _userError;
  }
  if (recommender) {
    if (GLOBAL_INTEREST_LIST.length === 0) {
      ...
      if (interestError) {
        return console.error("Error fetching interest registry:", interestError);
      }
      GLOBAL_INTEREST_LIST.push(...interestData.map((interest)=>interest.id));
    }
    const vectorize = (userInterests)=>{
      const vector = GLOBAL_INTEREST_LIST.map((id)=>userInterests.includes(id) ? 1 : 0);
      return vector;
    };
    const dot = (a, b)=>a.reduce((acc, val, i)=>acc + val * b[i], 0);
    ...
    const sourceVector = vectorize(sourceProfile?.profile_interests.map((i)=>i.interest_registry.id));
    console.log("Source vector for user ID", sourceId, "is", sourceVector);
    userData = userData.map((user)=>{
      const targetVector = vectorize(user.profile_interests.map((i)=>i.interest_registry.id));
      console.log("Target vector for user ID", user.id, "is", targetVector);
      console.log("Cosine similarity between source and target vectors:", cosineSimilarity(sourceVector, targetVector));
      return {
        ...user,
        similarity: user.id === sourceId ? 0 : cosineSimilarity(sourceVector, targetVector)
      };
...
export const getConnectedUserIds = async (userId, mode)=>{
  const { data: connectionData, error: connectionError } = await supabase.from("connections").select(`cohert2, cohert1`).or(`cohert1.eq.${userId},cohert2.eq.${userId}`).eq("type", mode);
  if (connectionError) {
    return {
      status: "success",
      response: []
...
const getLikedUserIds = async (userId)=>{
...
export const getDiscoveryProfiles = async (sourceId, filters)=>{
  const { response: connectedUsersIds } = await getConnectedUserIds(sourceId, "flatmate");
...
export const getConnectedUsers = async (sourceId, { minimal, mode })=>{
  const { response: connectedUserIds } = await getConnectedUserIds(sourceId, mode);
...
export const likeUser = async (targetId, sourceId)=>{
  if (targetId === sourceId) return {
    status: "error",
    response: "Invalid user"
  };
  const { data, error } = await supabase.from("profile_interactions").select("type").eq("cohert2", sourceId).eq("cohert1", targetId).eq("type", "like").single();
  if (!data) {
    ...
    const deleteInteractions = Promise.all([
      supabase.from("profile_interactions").delete().eq("cohert1", targetId).eq("cohert2", sourceId),
      supabase.from("profile_interactions").delete().eq("cohert1", sourceId).eq("cohert2", targetId)
    ]);
    const createConversation = supabase.from("conversation_registry").insert({}).select("conversation_id").single();
    const [_, { data: conversationData, error: conversationError }] = await Promise.all([
      deleteInteractions,
      createConversation
    ]);
    if (!conversationData) {
...
export const dislikeUser = async (targetId, sourceId)=>{
  if (targetId === sourceId) return {
    status: "error",
    response: "Invalid user"
  };
